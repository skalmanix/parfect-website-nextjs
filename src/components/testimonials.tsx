		...
		>
			<div className="container-wide section-padding">
				<div data-reveal className="text-center max-w-2xl mx-auto mb-12">
					<p className="eyebrow mb-4">Real couples</p>
					<h2
						id="testimonials-heading"
						className="font-display text-3xl sm:text-4xl md:text-5xl font-medium tracking-tight mb-4 text-balance"
					>
						It&apos;s already working for couples like you
					</h2>
					<div className="flex items-center justify-center gap-2 text-muted">
						<RatingStars />
						<span className="text-sm">{RATING_LINE}</span>
					</div>
				</div>
				...
				<div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-5 md:gap-6 mb-5 md:mb-6">
					...
					>
						<Image
							...
							alt={`${FEATURED_TESTIMONIAL.names}, a happy couple laughing together`}
							fill
						...
						<figcaption className="relative self-end p-6 md:p-8">
							...
							<p className="text-sm">
								<span className="font-medium">{FEATURED_TESTIMONIAL.names}</span>
								<span className="text-muted"> · {FEATURED_TESTIMONIAL.context}</span>
							</p>
						</figcaption>
					</figure>

					{/* First two quote cards */}
					<div className="grid gap-5 md:gap-6">
						{TESTIMONIALS.slice(0, 2).map((t, index) => (
							<TestimonialCard key={t.names} testimonial={t} delay={index * 80} />
						))}
					</div>
				</div>
...
function TestimonialCard({
	...
	return (
		...
		>
			<RatingStars className="mb-4" size="w-3.5 h-3.5" />
			...
			<figcaption className="flex items-center gap-3">
				...
				<span>
					<span className="block text-sm font-medium">{testimonial.names}</span>
					<span className="block text-xs text-muted">{testimonial.context}</span>
				</span>
			</figcaption>
		</figure>
	);
